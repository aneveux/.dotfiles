...
```bash
INIT=$(node "/home/antoine/.claude/get-shit-done/bin/gsd-tools.cjs" init phase-op "${PHASE}")
...
```bash
node "/home/antoine/.claude/get-shit-done/bin/gsd-tools.cjs" commit "docs(${padded_phase}): capture phase context" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
```bash
node "/home/antoine/.claude/get-shit-done/bin/gsd-tools.cjs" commit "docs(state): record phase ${PHASE} context session" --files .planning/STATE.md
```
</step>
...
```bash
node "/home/antoine/.claude/get-shit-done/bin/gsd-tools.cjs" config-set workflow._auto_chain_active true
```
