...
@/home/antoine/.claude/get-shit-done/references/git-integration.md
</required_reading>
...
```bash
INIT=$(node "/home/antoine/.claude/get-shit-done/bin/gsd-tools.cjs" init execute-phase "${PHASE}")
...
```bash
node "/home/antoine/.claude/get-shit-done/bin/gsd-tools.cjs" phases list --type summaries --raw
...
See `/home/antoine/.claude/get-shit-done/references/tdd.md` for structure.
</tdd_plan_execution>
...
See /home/antoine/.claude/get-shit-done/references/checkpoints.md for details.
</step>
...
If user_setup exists: create `{phase}-USER-SETUP.md` using template `/home/antoine/.claude/get-shit-done/templates/user-setup.md`. Per service: env vars table, account setup checklist, dashboard config, local dev notes, verification commands. Status "Incomplete". Set `USER_SETUP_CREATED=true`. If empty/missing: skip.
</step>

<step name="create_summary">
Create `{phase}-{plan}-SUMMARY.md` at `.planning/phases/XX-name/`. Use `/home/antoine/.claude/get-shit-done/templates/summary.md`.
...
node "/home/antoine/.claude/get-shit-done/bin/gsd-tools.cjs" state advance-plan

# Recalculate progress bar from disk state
node "/home/antoine/.claude/get-shit-done/bin/gsd-tools.cjs" state update-progress

# Record execution metrics
node "/home/antoine/.claude/get-shit-done/bin/gsd-tools.cjs" state record-metric \
...
node "/home/antoine/.claude/get-shit-done/bin/gsd-tools.cjs" state add-decision \
  --phase "${PHASE}" --summary-file "${DECISION_TEXT_FILE}" --rationale-file "${RATIONALE_FILE}"

# Add blockers if any found
node "/home/antoine/.claude/get-shit-done/bin/gsd-tools.cjs" state add-blocker --text-file "${BLOCKER_TEXT_FILE}"
```
</step>
...
```bash
node "/home/antoine/.claude/get-shit-done/bin/gsd-tools.cjs" state record-session \
...
```bash
node "/home/antoine/.claude/get-shit-done/bin/gsd-tools.cjs" roadmap update-plan-progress "${PHASE}"
```
...
```bash
node "/home/antoine/.claude/get-shit-done/bin/gsd-tools.cjs" requirements mark-complete ${REQ_IDS}
```
...
```bash
node "/home/antoine/.claude/get-shit-done/bin/gsd-tools.cjs" commit "docs({phase}-{plan}): complete [plan-name] plan" --files .planning/phases/XX-name/{phase}-{plan}-SUMMARY.md .planning/STATE.md .planning/ROADMAP.md .planning/REQUIREMENTS.md
```
</step>
...
```bash
node "/home/antoine/.claude/get-shit-done/bin/gsd-tools.cjs" commit "" --files .planning/codebase/*.md --amend
```
</step>
